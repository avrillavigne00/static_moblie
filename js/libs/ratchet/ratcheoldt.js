...
!(function () {
  ...
  var getModal = function (event) {
    ...
    if (modalToggle && modalToggle.hash&&!modalToggle.hasAttribute("data-ignore")){
    	return document.querySelector(modalToggle.hash);
    }
  };
  /*
  * 解决滚动事件与点击事件冲突
  * 判断如果横竖轴移动距离在15以内执行点击事件
  * 否则不执行点击事件，执行滚动事件
  */
  var startX = 1;
  var startY = 1; 
  ...
  window.addEventListener('touchstart', function (event) {
      endX =startX = event.touches[0].pageX;
      endY =startY = event.touches[0].pageY;
  });
  ...
  window.addEventListener('touchend', function (event) {    
    ...
    if (modal) {
      if (modal && modal.classList.contains('modal')) {
        if(0<=Math.abs(startX-endX)&&Math.abs(startX-endX)<15&&0<=Math.abs(startY-endY)&&Math.abs(startY-endY)<15)
